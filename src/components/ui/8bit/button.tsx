...
import { audioManager, SfxName } from "@/lib/audio/audio-manager";
...
function Button({
  ...
  const handleClick = (e: React.MouseEvent<HTMLButtonElement>) => {
    if (!props.disabled && soundEffect !== false) {
      audioManager.playSfx(soundEffect || 'uiClick');
    }
    onClick?.(e);
  };
  ...
  return (
    <ShadcnButton
      ...
      onClick={handleClick}
      className={cn(
        "rounded-none active:translate-y-1 transition-transform relative inline-flex items-center justify-center leading-none gap-1.5 border-none",
        size === "icon" && "mx-1 my-0",
        font !== "normal" && "retro",
        className
      )}
